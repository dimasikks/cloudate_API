...
do
  ...
  echo "Включаем ВМ"
  power_on=$(curl -s -X PUT "https://cloudate.digitalleague.ru/api/client/orders/$vm_ID" \
    --header "Content-Type: application/json" \
    --header "Token: $CLOUDATE_TOKEN" \
    --data '{"signal":"power_on"}' | jq -r .state)
  echo "Статус: $power_on"
  ...
  zabbix_hostid=$(curl -s -X POST "http://zabbix.sed.rtech.ru/api_jsonrpc.php" \
    ...
    echo "Включаем отслеживание ВМ в zabbix"
